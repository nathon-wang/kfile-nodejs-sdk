...
function KingFileSDK (opts) {
    this.options = opts;
    var debug_level = opts.debug === null ? 'error' : (opts.debug === '1' ? 'info': 'debug');
    this.logger = logger.colorConsole({level: debug_level});
    this.initialized = false;
    if (!this.options.device_id) {
        this.options.device_id = '123456';
    }
    if (!this.options.user_agent) {
        this.user_agent = 'kingsoft-ecloud-web;' + this.options.device_id + ';' + '0.1.0';
    } else {
        this.user_agent = this.options.user_agent;
    }
    return this;
}
    ...
    return new P(function (resolve, reject) {
        ...
        if (!_this.initialized) {
            ...
            request({url: _this.options.web_server_url, method: 'GET'}, function (err, response, body) {
                ...
                _this.logger.debug(util.format("Requesting api Url: %s, Response: %s", _this.options.web_server_url, body));
                if (!err && response.statusCode == 200) {
                    ...
                    if (web_config.code !== 0) {
                        ...
                        request(request_options, function (err, response, body) {
                            _this.logger.debug(util.format("Requesting api Url: %s, Response: %s", _this.options.api_server_info_url, body));
                            if (!err && response.statusCode == 200) {
...
exports.Role = require("./models/role").ROLE_MAP;
