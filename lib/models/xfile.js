...
function retrieve_path_n_xid(xfile_paths) {
    var pathNxid = [], path_set = {};
    xfile_paths.forEach(function (pitems) {
        var path = _.map(pitems, 'name').join('/'), data;
        if (path_set[path]) {
            return;
        }
        data = {path: './' + path, xid: pitems[pitems.length-1].xid};
        if (pitems[pitems.length-1].xtype === 0) {
            data.obj = _createXFileByProperties(null, null, {});
        }
        pathNxid.push(data);
        path_set[path] = true;
    });
...
function convert_n_create_path(dir_paths, file_paths) {
    var pathNxid = retrieve_path_n_xid(dir_paths),
    fileNxid = retrieve_path_n_xid(file_paths);
    return new P(function (resolve, reject) {
        P.map(pathNxid, function (item) {
            return mkdirp(item.path);
        })
        .then(function () {
            resolve({p2x: pathNxid, f2x: fileNxid});
        })
        .catch(function (error) {
            reject(error);
        });
...
function download_all_files(dir_paths, file_paths) {
    return P.map();
}

KDirectory.prototype.download = function () {
    var _this = this;
    return  new P (function (resolve, reject) {
        ...
        .then(function (snapshot) {
            ...
            return convert_n_create_path(dir_paths, file_paths);
        })
        .then(function (data) {
            return download_all_files(data);
        })
        .then(function () {
            resolve();
        })
