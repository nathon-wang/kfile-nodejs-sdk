...
function KFileBlockCrypto () {
    if (0 === 1) {
        this.key =  '';
        this.iv =  '';
        this.encryptor = crypto.createCipheriv('aes-256-cbc', new Buffer(this.key), this.iv);
        this.decryptor = crypto.createDecipheriv('aes-256-cbc', new Buffer(this.key), this.iv);
    } else {
...
exports.createEncryptStream = function () {
    return through2(function (chunk, enc, cb) {
        var err, data;
        if (! this.alg) {
            this.alg = new KFileBlockCrypto().encryptor;
        }
        try {
...
exports.createDecryptStream = function () {
    return through2(function (chunk, enc, cb) {
        var err, data;
        if (! this.alg) {
            this.alg = new KFileBlockCrypto().decryptor;
        }
        try {
            data = this.alg.update(chunk);
            this.push(data);
