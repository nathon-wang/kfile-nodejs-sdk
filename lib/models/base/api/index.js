...
function _retry_n_reject (opts) {
    ...
    .catch(function (error) {
        ...
        if (counter > max_retry) {
            ...
            console.log(util.format('Error %s happend, Retry %s, max %s, stack %s....', error, counter, max_retry, error.stack));
            var new_opts = {
                operation : operation,
                delay : delay*2 ,
                counter : counter+1,
...
function _api_call_promise(opts) {
    ...
    return new P(function (resolve, reject) {
        request(request_opts, function (err, response, body) {
            console.log(util.format("Requesting api Url: %s, Args: %s, Response: %s", request_opts.url, request_opts.body, body));
